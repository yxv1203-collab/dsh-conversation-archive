...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      '.dwm{color:var(--dsh-fg,var(--foreground,inherit));font:inherit;max-width:920px;line-height:1.5;padding:2px 0 28px}',
      '.dwm-head,.dwm-card-h,.dwm-actions,.dwm-fields,.dwm-row,.dwm-status,.dwm-modal-actions{display:flex;gap:8px}.dwm-head{align-items:flex-start;justify-content:space-between;margin:0 0 22px}.dwm-title{font-size:20px;font-weight:650;margin:0}.dwm-sub,.dwm-note,.dwm-meta,.dwm-row-sub,.dwm-card-desc{color:var(--dsh-muted,var(--muted-foreground,rgba(127,127,127,.8)));font-size:13px}.dwm-sub{margin:5px 0 0}',
      '.dwm-card{background:var(--dsh-panel,var(--card,rgba(127,127,127,.035)));border:1px solid var(--dsh-border,var(--border,rgba(127,127,127,.18)));border-radius:16px;margin:0 0 14px;padding:18px}.dwm-card-h{align-items:center;justify-content:space-between;margin-bottom:12px}.dwm-card-title{font-size:16px;font-weight:620}.dwm-card-desc{margin-top:2px}.dwm-actions{align-items:center;flex-wrap:wrap;justify-content:flex-end}.dwm-fields{align-items:end;flex-wrap:wrap}.dwm-toolbar{display:grid;gap:8px;grid-template-columns:minmax(220px,1fr) auto auto;margin:2px 0 4px}',
      '.dwm-btn,.dwm-input,.dwm-select{background:transparent;border:1px solid var(--dsh-border,var(--border,rgba(127,127,127,.28)));border-radius:10px;color:inherit;font:inherit;font-size:13px;padding:7px 10px}.dwm-btn{cursor:pointer}.dwm-btn:hover{background:var(--dsh-hover,rgba(127,127,127,.1))}.dwm-btn:disabled{cursor:not-allowed;opacity:.5}.dwm-btn:focus-visible,.dwm-input:focus-visible,.dwm-select:focus-visible{outline:2px solid var(--dsh-focus,var(--primary,#5b8cff));outline-offset:2px}.dwm-primary{background:var(--dsh-primary,var(--primary,#5b8cff));border-color:var(--dsh-primary,var(--primary,#5b8cff));color:var(--dsh-primary-fg,#fff)}.dwm-danger{color:var(--dsh-danger,#c85252)}.dwm-danger-solid{background:var(--dsh-danger,#c85252);border-color:var(--dsh-danger,#c85252);color:#fff}.dwm-input{min-width:190px}.dwm-search{width:100%}.dwm-field{display:grid;gap:4px}.dwm-label{color:var(--dsh-muted,var(--muted-foreground,rgba(127,127,127,.8)));font-size:12px}.dwm-icon{align-items:center;display:inline-flex;height:34px;justify-content:center;padding:0;width:34px}.dwm-icon svg{height:17px;stroke:currentColor;width:17px}.dwm-reminder{align-items:center;display:flex;font-size:12px;gap:6px;white-space:nowrap}.dwm-reminder .dwm-input{min-width:0;padding:5px 7px;text-align:center;width:54px}.dwm-details{border-top:1px solid var(--dsh-border,var(--border,rgba(127,127,127,.16)));margin-top:12px;padding-top:10px}.dwm-details summary{cursor:pointer;font-size:13px}',
      '.dwm-list{border-top:1px solid var(--dsh-border,var(--border,rgba(127,127,127,.16)));margin-top:13px;max-height:280px;overflow-y:auto}.dwm-row{align-items:center;border-bottom:1px solid var(--dsh-border,var(--border,rgba(127,127,127,.13)));min-height:55px;padding:8px 1px}.dwm-row-main{flex:1;min-width:0}.dwm-row-title{font-size:14px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.dwm-row-sub{margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.dwm-badge{background:var(--dsh-hover,rgba(127,127,127,.12));border-radius:999px;font-size:11px;padding:2px 7px;white-space:nowrap}.dwm-check{height:16px;width:16px}.dwm-empty{color:var(--dsh-muted,var(--muted-foreground,rgba(127,127,127,.8)));font-size:13px;padding:8px 0}',
    ...
    function WorkspaceSettings() {
      ...
      const backupRows = state.backups.map((item) => h('div', { className: 'dwm-row', key: item.id },
        ...
        h('button', { type: 'button', className: 'dwm-btn', disabled, onClick: (event) => openDialog({ title: '恢复此备份', description: '恢复前会重新验证 ZIP 清单和文件哈希。请先在下方填写一个已存在且为空的恢复目录。', confirm: '恢复到该目录', run: () => restoreTarget.trim() ? mutate('backupRestore', { id: item.id, targetDir: restoreTarget.trim() }) : Promise.resolve(setMessage('请先填写空的恢复目录。')) }, event) }, '恢复'),
      ))
      ...
      return h('div', { className: 'dwm' },
        ...
        h(Card, { title: '本地备份', description: '备份受管数据到本地或网络目录。', actions: h('button', { type: 'button', className: 'dwm-btn dwm-primary', disabled: disabled || !state.status?.settings?.backup?.configured || (backupScope !== 'all' && !backupScopeId), onClick: runBackup }, '立即备份') },
          h('div', { className: 'dwm-fields' },
            ...
            h('label', { className: 'dwm-field' },
              h('span', { className: 'dwm-label' }, '恢复目录（空文件夹）'),
              h('div', { className: 'dwm-actions' },
                h('input', { className: 'dwm-input', value: restoreTarget, placeholder: '恢复前填写一个空文件夹', disabled, onChange: (event) => setRestoreTarget(event.target.value) }),
                h('button', { type: 'button', className: 'dwm-btn', disabled: disabled || typeof workspaceRuntime?.pickDirectory !== 'function', onClick: async () => { const targetDir = await workspaceRuntime.pickDirectory(); if (targetDir) setRestoreTarget(targetDir) } }, '选择…'),
              ),
            ),
        ...
        h(Card, { title: '插件设置' }, h('div', { className: 'dwm-fields' },
          h('div', { className: 'dwm-field' }, h('span', { className: 'dwm-label' }, '工作区根目录'), h('span', { className: 'dwm-note' }, state.status?.settings?.harnessRoot || '正在读取…')),
