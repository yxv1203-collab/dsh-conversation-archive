...
assert.match(source, /\['session',\s*'单个对话缓存'\]/, 'offers active or archived session cache backup scope')
assert.match(source, /\['project',\s*'单个项目缓存'\]/, 'offers a project backup scope')
...
assert.match(source, /projectLabel.*项目缓存|项目缓存.*projectLabel/, 'uses safe project labels in archived-session disclosure')
