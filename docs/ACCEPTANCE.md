...
- 新会话仅登记 DSH 原生元数据，不创建镜像缓存、`.cache` 或文档/代码等分类目录；标题更新、会话结束、归档和恢复均不重排工作区文件。
- 删除前按会话时间窗口扫描原生工作区，排除依赖和临时文件后由 DSH 当前模型审核重要产出；旧版本已登记缓存仅保留兼容清理能力。
...
- 真实隔离 web profile 完整启动后，`/conversation-archive-api?action=diagnostics` 返回插件 `1.1.0`、DSH `0.1.1-rc.2`、原生归档/恢复/会话定位均可用且非只读；`/plugins/dsh-conversation-archive/client.js` 返回 200 并包含“工作区管理”。随后已终止进程并删除临时 DSH_HOME。
