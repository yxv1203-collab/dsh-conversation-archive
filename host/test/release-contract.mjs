...
assert.equal(manifest.dsh.maxTestedVersion, '0.1.2-rc.1')
...
assert.match(readmeEnglish, /profiles[\\/]web[\\/]deps/i, 'English offline install keeps the tarball in the DSH profile')
assert.match(readmeChinese, /profiles[\\/]web[\\/]deps/i, 'Chinese offline install keeps the tarball in the DSH profile')
assert.doesNotMatch(readmeEnglish, /plugin --profile web add \.\\dsh-conversation-archive/i, 'English README never installs from a disposable download path')
assert.doesNotMatch(readmeChinese, /plugin --profile web add \.\\dsh-conversation-archive/i, 'Chinese README never installs from a disposable download path')
