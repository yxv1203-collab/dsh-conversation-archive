...
工作区管理直接集成在 DSH 原生设置系统中，并保持一致的视觉与交互方式。

![DSH 工作区管理中的已归档对话与保留文件](docs/images/workspace-manager-overview.png)

*归档搜索与筛选、批量操作，以及跨对话共享的保留文件库。*

![DSH 工作区管理中的本地备份与插件设置](docs/images/backup-and-settings.png)

*本地或网络备份目录、自动备份、版本保留、更新检查与诊断状态。*

### 归档元数据状态

**元数据未登记**表示该原生归档项没有插件登记的文件映射，常见于安装插件前已归档或只存在于 DSH 原生状态中的对话。这不是故障；原生恢复和删除仍可使用，插件只是没有该对话额外登记的文件范围需要保留或清理。
