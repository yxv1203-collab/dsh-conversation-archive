...
面向 DeepSeek Harness（DSH）的原生工作区治理插件，将已归档对话管理、删除前重要文件保护与可验证本地备份统一集成到设置界面。插件以 DSH 原生会话状态为准，并通过严格的路径边界校验、内容哈希与失败关闭机制，降低清理历史会话时误删重要产出或产生不可恢复状态的风险。

## 主要功能

- **原生归档管理：**集中浏览、搜索和筛选 DSH 已归档对话，支持单项或批量取消归档与删除；归档状态始终以 DSH 原生记录为唯一依据，不维护平行索引。
- **重要产出保护：**删除前由 DSH 当前配置的模型审核符合条件的工作区产出，将选中文件写入共享去重保留库，并以内容哈希验证副本完整性；同时支持恢复、批量移除与应用内提醒。
- **可验证本地备份：**支持手动、定时及正常关闭前生成 ZIP 备份。每份备份均包含文件清单与哈希，只能恢复到空目录且不会覆盖现有文件；自动备份默认关闭，默认保留最近五份已验证备份。
- **工作区与路径安全：**支持跨磁盘的 DSH 已登记工作区和独立项目目录，可从原生元数据自动补齐历史映射，并拒绝通过 Junction 或符号链接越过受管路径边界。
